...
export async function generateAndSendOTP(
  ...
  try {
    ...
    if (sent) {
      ...
      return {
        success: false,
        message: 'Unable to send verification code at this time. Please try again later or contact customer service at +233 50 129 0952.',
        sessionKey: ''
      };
    }
  } catch (error: any) {
...
export function verifyOTP(
  ...
  if (session.attempts > MAX_OTP_ATTEMPTS) {
    ...
    return {
      success: false,
      message: 'Maximum verification attempts exceeded. Please request a new code or contact customer service at +233 50 129 0952.'
    };
  }
