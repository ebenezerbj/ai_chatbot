<?php
/**
 * PDF Downloader - Handles both file-based and database blob PDFs
 * Usage: download_db.php?id=123 (for database) OR download_db.php?path=sitedoc/file.pdf (for files)
 */
...
if ($useDatabase) {
    ...
    try {
        ...
        if (strlen($document) > 500) {
            ...
            // It's a filename, redirect to file-based downloader
            $filePath = 'sitedoc/' . $document;
            header('Location: download1.php?path=' . urlencode($filePath));
            exit;
        }
...
// Handle file-based (same as download1.php)
if ($useFile) {
    $path = $_GET['path'];
